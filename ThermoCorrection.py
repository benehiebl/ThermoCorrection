...
def FvFromVI (VI_array,soil_limit,vegetation_limit):
    ...
    fv_cover = np.where(VI_array > soil_limit, ((VI_array-soil_limit)/(vegetation_limit-soil_limit))**2,fv_cover)
...
def LseFromFv (fv_cover,veg_lse,soil_lse,cavity):
    ...
    lse = np.where(fv_cover != -99999, veg_lse*fv_cover + soil_lse*(1-fv_cover) + (4 * cavity * fv_cover*(1-fv_cover)),fv_cover)
...
def Boltz():
    o = (5454781984210512994952000000 * (m.pi)**5)/29438455734650141042413712126365436049
    return o
...
cam_location = (float(pdict["CAMLOCATION_X"]),float(pdict["CAMLOCATION_Y"]),float(pdict["CAMLOCATION_Z"]))

# station_height = float(pdict["STATION_HEIGHT"])
# station_data = pdict["STATION_DATA"]
vertical_gradient = pdict["VERTICAL_GRAD"]

em_const = float(pdict["EM_CONST"])
#soil_threshold = (float(pdict["SOIL_THRESHOLD"]),float(pdict["VEG_THRESHOLD"]))
soil_lse = (float(pdict["LSE_VEGETATION"]),float(pdict["LSE_SOIL"]))

### assigning values vor atmospheric radiance (pathradiance) and downwelling radiance (atmosphärische gegenstrahlung)
L_atm = pdict["RADIANCE_AIR"]
if L_atm == "NA":
    L_atm = 0
else:
    L_atm = float(L_atm)
L_down = pdict["RADIANCE_DOWN"]
if L_down == "NA":
    L_down = 324
else:
    L_down = float(L_down)
...
files_thermo = FileList(thermo_loc)
...
# read meteo data at camera 
#meteo = pandas.read_csv(station_data,sep=",")
# read meteo vertical profile
gradient = pandas.read_csv(vertical_gradient,sep=",")
# read time info for thermo raster
timeinfo = pandas.read_csv(time_info,sep=",")
...
for i in range(0,len(gradient["time"])):
    time_val = 24*60*int(gradient["time"][i][:2]) + 60*int(gradient["time"][i][11:13]) + int(gradient["time"][i][14:16])
...
for i in range(0,len(timeinfo["Datetime"])):
    time_val = 24*60*int(timeinfo["Datetime"][i][:2]) + 60*int(timeinfo["Datetime"][i][11:13]) + int(timeinfo["Datetime"][i][14:16])
...
if red_name != "None":
    with rasterio.open(red_name,"r") as img:
        red = img.read(1)
    lse_veg = np.where(red >= 250, 0.98, lse_veg)
else:
...
ground_dist = np.where(dsm != -99999, np.sqrt((cam_location[0]-x_ar)**2 + (cam_location[1]-y_ar)**2), -99999)      
height_diff = np.where(dsm != -99999, np.sqrt((dsm - cam_location[2])**2), -99999)        

# Calculate path distance between XYZ and cam location
path_ar = np.where(dsm != -99999, np.sqrt(ground_dist**2 + height_diff**2), -99999)        
...
for nr in range(0,len(files_thermo)):
    ...
    idx2 = pandas.Index(gradient["timeindex"])
    gradientindex = idx2.get_loc(thermo_timeindex, method="ffill")
    # get 
    T_gradient = gradient["TL_coefficient"][gradientindex] + (thermo_timeindex - gradient["timeindex"][gradientindex]) * ((gradient["TL_coefficient"][gradientindex+1]-gradient["TL_coefficient"][gradientindex]) /(gradient["timeindex"][gradientindex+1]-gradient["timeindex"][gradientindex]))
    T_intercept = gradient["TL_intercept"][gradientindex] + (thermo_timeindex - gradient["timeindex"][gradientindex]) * ((gradient["TL_intercept"][gradientindex+1]-gradient["TL_intercept"][gradientindex]) /(gradient["timeindex"][gradientindex+1]-gradient["timeindex"][gradientindex]))

    RH_gradient = gradient["RH_coefficient"][gradientindex] + (thermo_timeindex - gradient["timeindex"][gradientindex]) * ((gradient["RH_coefficient"][gradientindex+1]-gradient["RH_coefficient"][gradientindex]) /(gradient["timeindex"][gradientindex+1]-gradient["timeindex"][gradientindex]))
    RH_intercept = gradient["RH_intercept"][gradientindex] + (thermo_timeindex - gradient["timeindex"][gradientindex]) * ((gradient["RH_intercept"][gradientindex+1]-gradient["RH_intercept"][gradientindex]) /(gradient["timeindex"][gradientindex+1]-gradient["timeindex"][gradientindex]))
    ...
    T_pix = np.where(dsm != -99999, (T_gradient * dsm + T_intercept) + 271.15, -99999)  
    pathtemp = np.where(dsm != -99999, (T_pix + T_cam) / 2, -99999)
    ...
    #########################################################################################################################################
    ###Calculating Atmospheric transmission from Wiecek (2011)
    
    ### Calculating constant alpha from passman larmore tables (experimentally) wavelength 7.5-13ym
    ...
    ### Correction model
    #############################################################################################
    print("      Start correcting "+files_thermo[nr])
    ### Corection based on Gillespie 1998 and Richter 2002
    
    with rasterio.open(thermo_loc+"/"+files_thermo[nr]) as img:
        thermo1 = img.read(1)
        profile_thermo = img.profile
        
    # downwelling radiance and path radiance are still missing!!!!
    # reflected downwelling LWIR = (1-lse)*downradiance, atmosphärische gegenstrahlung ~324 W/m^-2
    
    # path radiance = LWIR atmosphärische eigenstrahlung corresponding to pathdistance
    L_path = L_atm * path_ar 
       
    ### calculate at cam radiance from thermo   
    # check if in Kelvin or degree
    mean_th = np.mean(thermo1)
    if mean_th < 100:
        thermo = np.where(thermo1 != -99999, thermo1 + 271.15, -99999)
    else:
        thermo = thermo1
        
    # at cam radiance
    L_cam = np.where(thermo != -99999, Boltz() * (thermo)**4, -99999)
       
    ### Correcting Surface radiance for each pixel
    L_surf = np.where(thermo > 0, (L_cam - L_path - tao_atm*(1 - lse) * (L_down/m.pi)) / (tao_atm * lse), np.nan)
    
    ### Surface temperature in different units
    if mean_th < 100:
        T_surf = (L_surf / Boltz())**(1/4) - 271.15
    else:
        T_surf = (L_surf / Boltz())**(1/4)
    
    # save corrected surface temperature
    print("      Saving...")  
    profile_thermo["dtype"] = T_surf.dtype
    with rasterio.open(f"OUTPUT/{files_thermo[nr]}", "w", **profile_thermo) as dst:
        dst.write(T_surf,1)
...
with rasterio.open("OUTPUT/LSE.tif", "w", **profile_dsm) as dst:
    dst.write(lse,1)
    
# save tao
profile_thermo["dtype"] = tao_atm.dtype
with rasterio.open("OUTPUT/Tao.tif", "w", **profile_thermo) as dst:
    dst.write(tao_atm,1)

# profile_thermo["dtype"] = path_ar.dtype
# with rasterio.open("OUTPUT/path_ar.tif", "w", **profile_thermo) as dst:
#     dst.write(path_ar,1)

# profile_thermo["dtype"] = pathRH.dtype
# with rasterio.open("OUTPUT/pathRH.tif", "w", **profile_thermo) as dst:
#     dst.write(pathRH,1)

# profile_thermo["dtype"] = L_cam.dtype
# with rasterio.open("OUTPUT/thermo.tif", "w", **profile_thermo) as dst:
#     dst.write(L_cam,1)
