...
def LseFromFv(fv_cover, veg_index, veg_lse, soil_lse, cavity):
...
lse_veg = LseFromFv(fv_cover, vi, soil_lse[0], soil_lse[1], em_const)
...
for nr in range(0, len(files_thermo)):
    ...
    T_pix = np.where(dsm != -99999, (T_gradient *
                     dsm + T_intercept) + 271.15, -99999)
    pathtemp = np.where(dsm != -99999, ((T_pix + T_cam) / 2) - 271.15, -99999)
    ...
    index_passman = [7.5, 8, 9, 10, 11, 12, 13]
    cols_passman = [0, 0.2, 0.5, 1, 2, 5, 10, 20]
    ...
    h_ar = (0.00016667 * pathtemp**3 + 0.01 * pathtemp**2 + 0.38333 * pathtemp + 5) * (pathRH/100) * path_ar

    p_h2o = np.ones(np.shape(dsm))
    p_co2 = np.ones(np.shape(dsm))
    tao_atm_list = []

    # iterating over all wavelengths
    for lamda in index_passman:
        print(lamda)
        for nr1 in range(0, len(cols_passman)-1):
            
            ## linearly interpolate values in passman larmore tables
            # for H2O            
            p_h2o = np.where((h_ar > cols_passman[nr1]) & (h_ar <= cols_passman[nr1+1]), ((h_ar - cols_passman[nr1]) / (cols_passman[nr1+1] - cols_passman[nr1])) * (
            ...
            # for CO2        
            p_co2 = np.where((path_ar > cols_passman[nr1]) & (path_ar <= cols_passman[nr1+1]), ((path_ar - cols_passman[nr1]) / (cols_passman[nr1+1] - cols_passman[nr1])) * (
    ...
    # calculate mean value over full spectrum (7.5 to 13 microns)
    tao_atm = np.array(tao_atm_list).mean(axis=0)
    ...
    # Corection based on Gillespie 1998 and Richter 2002 and atmospheric transfer equation
    ...
    # path radiance = LWIR atmosphärische eigenstrahlung corresponding to pathdistance/km
    ...
    # - L_path - tao_atm * (1 - lse) * (L_down/m.pi)
    L_surf = np.where((thermo > 0) & (lse != -99999), (L_cam) / (tao_atm * lse), np.nan)  # *tao_atm
