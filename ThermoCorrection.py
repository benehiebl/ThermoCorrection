...
for nr in range(0, len(files_thermo)):
    ...
    if mean_th < 100:
        thermo = np.where(thermo1 != -99999, thermo1 + 273.15, -99999)
    else:
        thermo = thermo1
    ...
    for lamda in index_passman:
        ...
        L_down = np.where(thermo != -99999, ((0.23 + 0.433 * (((RH_pix/100) * p_sat) / T_pix)**(1/8)) * (1 - cloud_fraction**2) + 0.976 * cloud_fraction**2) * (1.19104*10**8) / (lamda**5 * np.exp(14387.7 / (lamda * T_pix)) - 1), -99999)   #Boltz() * T_pix**4
        
        
        ### path radiance = LWIR atmosphärische eigenstrahlung corresponding to pathdistance/km Brutsaert 1975 and Siqueira Katul 2009
        L_path = e_h2o * (1-tao_lamda) * (1.19104*10**8) / (lamda**5 * np.exp(14387.7 / (lamda * (pathtemp + 273.15))) - 1)   #* Boltz() * (pathtemp + 273.15)**4
        ...
        L_lamda = np.where(thermo != -99999, (1.19104*10**8) / (lamda**5 * np.exp(14387.7 / (lamda * thermo)) - 1), -99999)
        # Correcting Surface radiance for each pixel
        L_lamda_surf = np.where((thermo > 0) & (lse != -99999), (L_lamda - L_path - tao_lamda * (1 - lse) * (L_down/m.pi)) / (tao_lamda * lse), np.nan)
        # calculate T by reverse plancks law
        T_corr = np.where(L_lamda_surf != -99999, 14387.7 / (lamda * np.log(((1.19104*10**8)/(lamda**5*L_lamda_surf)) +1)), -99999)        
        T_surf_all.append(T_corr)
    
    
    #### calculate mean temeperature over full wavelength spectrum   
    T_surf = np.array(T_surf_all).mean(axis = 0)
    del T_surf_all
